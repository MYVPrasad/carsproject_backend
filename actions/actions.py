...
class ActionGetCarInfo(Action):
    ...
    def run(self, dispatcher: CollectingDispatcher, tracker: Tracker, domain: Dict[Text, Any]) -> List[Dict[Text, Any]]:
        ...
        response = requests.get(api_url, headers={'X-Api-Key': 'kefmhL/pGay7C67rFefnBw==PfZTpNVnyRbSRfxy'})
        if response.status_code == requests.codes.ok:
            ...
            if car_info: 
                ...
                dispatcher.utter_message(text=f"your searched car details are car{make}the model is {model} it is manufactered in the year {year}.With the number of cylinders: {cylinders} it runs with the fuel type of: {fuel_type} and its milage on highways is  {highway_mpg} mpg")
            else:
